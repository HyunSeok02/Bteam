    ...
    public void btn3Clicked(View v){
        if(isNetworkConnected(this) == true){
            if(selItem != null){
               ...
               ListDelete listDelete = new ListDelete(selItem.getId(), selItem.getImage_path());        // 아이디와 이미지경로를 넘긴다.
                listDelete.execute();

                // 화면갱신
                Intent refresh = new Intent(this, Sub1.class);              // sub1을 한개더 띄우고
                startActivity(refresh);
                this.finish(); // 화면끝내기                             // 기존것 제거
